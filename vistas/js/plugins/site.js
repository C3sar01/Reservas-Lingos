...
window.Rainbow = (function () {
  ...
  function C(a, b) {
    for (var c in e[d]) {
      c = parseInt(c, 10);
      if (a == c && b == e[d][c] ? 0 : a <= c && b >= e[d][c])
        delete e[d][c], delete j[d][c];
      if ((a >= c && a < e[d][c]) || (b > c && b < e[d][c])) return !0;
    }
    return !1;
  }
  function r(a, b) {
  ...
  function o(a, b, c) {
    if (b < a.length) {
      var d = a[b],
        f = B(d);
      return !(-1 < (" " + d.className + " ").indexOf(" rainbow ")) && f
        ? ((f = f.toLowerCase()),
          (d.className += d.className ? " rainbow" : "rainbow"),
          n(d.innerHTML, f, function (k) {
            d.innerHTML = k;
            j = {};
            e = {};
            p && p(d, f);
  ...
  function A(a, b) {
    ...
    for (f = 0; f < c.length; ++f)
      c[f].getElementsByTagName("code").length || e.push(c[f]);
    o(e, 0, b);
  }
  var j = {},
    e = {},
...
window.addEventListener
  ? window.addEventListener("load", Rainbow.color, !1)
...
Rainbow.extend([
  ...
  },
  {
    matches: { 1: "keyword" },
    pattern:
      /\b(and|array|as|bool(ean)?|c(atch|har|lass|onst)|d(ef|elete|o(uble)?)|e(cho|lse(if)?|xit|xtends|xcept)|f(inally|loat|or(each)?|unction)|global|if|import|int(eger)?|long|new|object|or|pr(int|ivate|otected)|public|return|self|st(ring|ruct|atic)|switch|th(en|is|row)|try|(un)?signed|var|void|while)(?=\(|\b)/gi,
  },
